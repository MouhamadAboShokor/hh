...
%turn the sigma matrix to a single vector by stacking the diagon 
%of each component into a single "super diagon"
[ndim, nmix] = size(ubm.mu);
S = reshape(ubm.sigma, ndim * nmix, 1);

%extract the baum-welch statistics for each file "just programatically"
% and stores the in a single matrix i.e. :
% N is numFiles × M 
% F is numFiles × M.D
[N, F] = load_data(dataList, ndim, nmix);
%then remove the original cell array to save space 
if iscell(dataList), clear dataList; end

fprintf('\n\nRandomly initializing T matrix ...\n\n');
% suggested in JFA cookbook
% this T corresspondes to T' in the paper
...
% here we are done and we have our total-variability matrix 
if ( nargin == 6 ),
...
% this is (U' * Sigma^-1)    in equation 5
T_invS =  bsxfun(@rdivide, T, S');

% this is for prallelizing the code
...
for batch = 1 : nbatch,
    ...
    len = fin - start + 1;
    index = start : fin;
    % get the statistics for this batch 
    N1 = N(index, :);
    F1 = F(index, :);
...
%     for ix = 1 : len,
        % equation 5.1 just in a weird way 
        L = I +  bsxfun(@times, T_invS, N1(ix, idx_sv)) * T';
        Cxx = pinv(L); % this is the posterior covariance Cov(x,x)
        % equation 5.2 but for some reason the centered statistics 
        % got replaced by the first B-W statistics 
        B = T_invS * F1(ix, :)';
        %equations 8.1 and 8.2 just without the statistics  
        Ex(:, ix) = Cxx * B; % this is the posterior mean E[x]
        Exx(:, :, ix) = Cxx + Ex(:, ix) * Ex(:, ix)';
    end
    % then multiply by the first and second statistics
    RU = RU + Ex * F1;
...
for mix = 1 : nmix
    ...
    % just F.Y.I the term A\B means B*inv(A)
    % therefore this is eq 7
