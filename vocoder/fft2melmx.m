function [wts,binfrqs] = fft2melmx(nfft, sr, nfilts, width, minfrq, maxfrq, htkmel, constamp,linear,rect)
...
if nargin < 9;     linear=0;    end
if nargin < 10;     rect=0;    end
...
binfrqs = mel2hz(minmel+[0:(nfilts+1)]/(nfilts+1)*(maxmel-minmel), htkmel);
    ...
    halfFilterWidth=(maxfrq-minfrq)/(nfilts+1);
    binfrqs=[minfrq:halfFilterWidth:maxfrq];
end
...
%  fs = mel2hz(i + [-1 0 1], htkmel);
  ...
  if(rect)
    first=round(fs(1)*nfft/sr);
    last=round(fs(3)*nfft/sr);
    wts(i,1+[0:(nfft/2)]) = [zeros(1,first),...
        ones(1,last-first),zeros(1,((nfft/2)+1)-last)];
  else
